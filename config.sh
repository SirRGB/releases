#!/bin/bash

export GITHUB_USER="SirRGB"
export GITHUB_EMAIL="sirrgb@proton.me"

export device="pdx203"

export ROM="crDroid"
export ROM_DIR="${WORKSPACE}/../Crd12"
export ROM_VERSION="8"
export local_manifest_url="https://raw.githubusercontent.com/SirRGB/local_manifests/main/pdx203/A12Lineage2.xml"
export manifest_url="https://github.com/crdroidandroid/android"
export rom_vendor_name="lineage"
export branch="12.1"
export bacon="bacon"
export buildtype="userdebug"
export clean="installclean"
export generate_incremental="false"
export upload_recovery="true"

export ccache="false"
export ccache_size=""

export jenkins="false"

export release_repo="SirRGB/releases"

export timezone="UTC"
