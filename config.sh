#!/bin/bash

export GITHUB_USER="SirRGB"
export GITHUB_EMAIL="sirrgb@proton.me"

export device="a5y17lte"

export ROM="Lineage OS"
export ROM_DIR="${WORKSPACE}/../Los12"
export ROM_VERSION="19.1"
export local_manifest_url="https://raw.githubusercontent.com/SirRGB/local_manifests/main/a57y17lte/A12Lineage2.xml"
export manifest_url="https://github.com/droid-legacy/android"
export rom_vendor_name="lineage"
export branch="lineage-19.1"
export bacon="bacon"
export buildtype="userdebug"
export clean="installclean"
export generate_incremental="false"
export upload_recovery="yes"

export ccache="false"
export ccache_size=""

export jenkins="false"

export release_repo="SirRGB/releases"

export timezone="UTC"
